...
public class ControladorCafeteria {
      private final  CafeteriasDAO dao;

    public ControladorCafeteria() {
        this.dao =new CafeteriasDAO();
    }
    ...
    public void guardarRestaurante(
        ...
        if (horaAperturaStr == null || horaAperturaStr.trim().isEmpty()) {
         ...
         if (horaCierreStr == null || horaCierreStr.trim().isEmpty()) {
          ...
          if (capacidad ==0) {
           ...
           if (tematica.isEmpty()) {
            throw new IllegalArgumentException("La tematica es obligatoria.");
        }
        ...
        try ( Connection con = ConexionHuellasCuencanas.conectar()) {
            try {
                ...
                cafeteria.setId_negocio(negocioId);
                cafeteria.setTematica(tematica);
                CafeteriasDAO guardar =new CafeteriasDAO();
                guardar.insertar(con, cafeteria);
    ...
    public Cafeterias obtenerDetalleCafeteria(int lugarInteresId) throws SQLException {
        CafeteriasDAO cafeteria = new CafeteriasDAO();
        return cafeteria.obtenerDetalleCafeteria(lugarInteresId);
    }
    
    public boolean actualizarCafeteriaCompleto(
               int capacidad,
           String tematica,
            int negocioid,
            String nombre,
       ...
       return dao.actualizarCompleto(capacidad, tematica, negocioid, nombre, direccion, latitud, longitud, descripcion, horaApertura, horaCierre, img1, img2, img3);
    }
}
