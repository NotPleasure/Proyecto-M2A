 ...
 * @author USER
 */
public class Ventana_AdministrarParques extends javax.swing.JFrame {

    private List<ParqueVista> listaParquesCompleta = new ArrayList<>();
    private Persona personaLogueada;
    private boolean sinBordes = true;
    private JFrame ventanaDestino;

    public Ventana_AdministrarParques(JFrame padre, boolean sinBordes) {
    ...
    private void cargarTablaParques() {
        try {
            ParqueDao dao = new ParqueDao();
            listaParquesCompleta = dao.buscarSoloTexto("");
            poblarTablaParques(listaParquesCompleta);
        } catch (SQLException ex) {
            ex.printStackTrace();
            JOptionPane.showMessageDialog(this,
                    "No se ha podido cargar los parques",
                    "Error BD",
    ...
    private void poblarTablaParques(List<ParqueVista> lista) {
        String[] columnas = {"ID", "Nombre", "Entidad gestora", "Superficie (ha)"};
        DefaultTableModel modelo = new DefaultTableModel(null, columnas);

        for (ParqueVista p : lista) {
            Object[] fila = {
                p.getId(),
                p.getNombre(),
                p.getEntidad_gestora(),
                p.getSuperficie()
            };
            modelo.addRow(fila);
        }
    ...
    private void inicializarFiltroParques() {
        ParqueDao dao = new ParqueDao();

        Busqueda2.getDocument().addDocumentListener(new DocumentListener() {
            private void filtrar() {
                String texto = Busqueda2.getText().trim().toLowerCase();
                try {
                    List<ParqueVista> filtrada = texto.isEmpty()
                            ? listaParquesCompleta
                            : dao.buscarSoloTexto(texto);
                    poblarTablaParques(filtrada);
                } catch (SQLException ex) {
                    ex.printStackTrace();
                }
            }
    ...
    public void recargarTablaParques() {
        cargarTablaParques();
    }
    ...
    private void mostrarConfirmacionEliminarParque(int id, String nombre) {
        Ventana_EliminarParque panel = new Ventana_EliminarParque(this);

        panel.setidParque(id);
    ...
    private void initComponents() {
        ...
        Gestion.setBackground(new java.awt.Color(21, 115, 59));
        Gestion.setForeground(new java.awt.Color(21, 115, 59));
        Gestion.setText("GESTIÓN DE PARQUES");
        ...
        jLabel4.setIcon(new javax.swing.ImageIcon(getClass().getResource("/Imágenes/Fondo Administrar Imágenes.png"))); // NOI18N
    ...
    private void ActualizarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ActualizarActionPerformed
        ...
        if (fila == -1) {
            JOptionPane.showMessageDialog(
                    this,
                    "Seleccione un parque de la tabla",
                    "Atención",
        ...
        int lugarInteresId = (int) jTable1.getValueAt(fila, 0);

        try {
            ParqueDao dao = new ParqueDao();
            ParqueDetalleVista vista = dao.obtenerDetalleParque(lugarInteresId);

            if (vista == null) {
                JOptionPane.showMessageDialog(
                        this,
                        "No se encontraron datos del parque seleccionado.",
                        "Error",
                        JOptionPane.ERROR_MESSAGE
                );
                return;
            ...
            Ventana_ActualizarParque panel = new Ventana_ActualizarParque(this);
            panel.cargarDatosParque(vista);
            GlassPanePopup.showPopup(panel);
            ...
            JOptionPane.showMessageDialog(
                    this,
                    "Error al cargar datos del parque:\n" + ex.getMessage(),
                    "Error",
                    JOptionPane.ERROR_MESSAGE
            );
        }
    ...
    private void EliminarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_EliminarActionPerformed
        ...
        mostrarConfirmacionEliminarParque(lugarInteresId, nombre);
