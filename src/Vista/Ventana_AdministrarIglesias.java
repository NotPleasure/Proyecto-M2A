 ...
 * @author USER
 */
public class Ventana_AdministrarIglesias extends javax.swing.JFrame {

    private List<IglesiaVista> listaIglesiasCompleta = new ArrayList<>();
    private Persona personaLogueada;
    private boolean sinBordes = true;
    private JFrame ventanaDestino;

    public Ventana_AdministrarIglesias(JFrame padre, boolean sinBordes) {
    ...
    private void cargarTablaIglesias() {
        try {
            IglesiaDAO dao = new IglesiaDAO();
            listaIglesiasCompleta = dao.buscarSoloTexto("");
            poblarTablaIglesias(listaIglesiasCompleta);
        } catch (SQLException ex) {
            ex.printStackTrace();
            System.out.println("No se ha podido cargar las iglesisas");
        }
    }

    // 2) Método que recibe la lista y la dibuja
    private void poblarTablaIglesias(List<IglesiaVista> lista) {
        String[] columnas = {"ID", "Nombre", "Apertura", "Cierre"};
        DefaultTableModel modelo = new DefaultTableModel(null, columnas);

        for (IglesiaVista ig : lista) {
            Object[] fila = {
                ig.getId(),
                ig.getNombre(),
                ig.getHoraApertura(),
    ...
    private void inicializarFiltroBusqueda() {
        ...
        Busqueda2.getDocument().addDocumentListener(new DocumentListener() {
            private void filtrar() {
                ...
                try {
                    List<IglesiaVista> listaFiltrada;
                    if (texto.isEmpty()) {
                        listaFiltrada = listaIglesiasCompleta;
                    } else {
                        listaFiltrada = dao.buscarSoloTexto(texto);
                    }
                    poblarTablaIglesias(listaFiltrada);
                } catch (SQLException ex) {
                    ex.printStackTrace();
                }
            }
    ...
    public void recargarTablaIglesias() {
        cargarTablaIglesias();
    }
    ...
    private void mostrarConfirmacionEliminarIglesia(int id, String nombre) {
        Ventana_EliminarIglesia panel = new Ventana_EliminarIglesia(this);
        panel.setIdIglesia(id);
    ...
    private void initComponents() {
        ...
        Gestion.setText("GESTIÓN DE IGLESIAS");
        ...
        jLabel4.setIcon(new javax.swing.ImageIcon(getClass().getResource("/Imágenes/Fondo Admin Iglesias 2.png"))); // NOI18N
  ...
  int fila = jTable1.getSelectedRow();
    if (fila == -1) {
        JOptionPane.showMessageDialog(
            this,
            "Seleccione una iglesia de la tabla",
            "Atención",
    ...
    try {
        IglesiaDAO dao = new IglesiaDAO();
        IglesiaDetalleVista vista = dao.obtenerDetalleIglesia(lugarInteresId);

        if (vista == null) {
            JOptionPane.showMessageDialog(
                this,
                "No se encontraron datos de la iglesia seleccionada.",
                "Error",
            ...
            return;
              ...
              Ventana_ActualizarIglesia panel = 
            new Ventana_ActualizarIglesia(this); 

        panel.cargarDatosIglesia(vista);
        ...
        JOptionPane.showMessageDialog(
            this,
            "Error al cargar datos de la iglesia:\n" + ex.getMessage(),
            "Error",
            JOptionPane.ERROR_MESSAGE
        );
    }
    ...
    private void EliminarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_EliminarActionPerformed
        int fila = jTable1.getSelectedRow();
        if (fila == -1) {
            JOptionPane.showMessageDialog(this, "Seleccione una iglesia de la tabla");
            return;
        }

        int lugarInteresId = (int) jTable1.getValueAt(fila, 0);
        System.out.println("ID seleccionado a eliminar: " + lugarInteresId);

        String nombre = jTable1.getValueAt(fila, 1).toString();
        mostrarConfirmacionEliminarIglesia(lugarInteresId, nombre);
